...
class Binary_distillation(QtWidgets.QDialog):
    ...
    def __init__(self, indices=None, nombres=None, x=None, y=None, parent=None):
        ...
        layout.addWidget(QtWidgets.QLabel(
            QtWidgets.QApplication.translate("equipment", "Component 1:")), 1, 1)
        self.Comp1 = QtWidgets.QComboBox()
        layout.addWidget(self.Comp1, 1, 2)
        layout.addWidget(QtWidgets.QLabel(
            QtWidgets.QApplication.translate("equipment", "Component 2:")), 1, 4)
        ...
        self.plot = mpl()
        tab.addTab(self.plot, QtWidgets.QApplication.translate(
            "equipment", "Plot"))
        self.tabla = Tabla(2, horizontalHeader=[
                           "x", "y"], stretch=False, readOnly=True)
        tab.addTab(self.tabla, QtWidgets.QApplication.translate(
            "equipment", "Table"))
        ...
        if ind1 != ind2:
            ...
            for z in zi:
                try:
                    ...
                    fraccion[ind1] = z
                    fraccion[ind2] = 1-z
                    mez = Mezcla(tipo=3, fraccionMolar=fraccion,
                                 caudalMasico=1.)
...
class Plot_Distribucion(mpl):
    ...
    def __init__(self, id, solido, parent=None):
        ...
        self.setWindowTitle(QtWidgets.QApplication.translate(
            "pychemqt", "Stream")+" "+str(id)+" - "+self.title)
        self.fill(solido)
        self.axes2D.legend(loc=4)
        self.axes2D.set_ylim(0, 1)
        self.axes2D.set_xlabel("Dp, "+unidades.Length(None).text(
            "ParticleDiameter"), horizontalalignment='right', size='12')
        self.axes2D.set_ylabel(QtWidgets.QApplication.translate(
            "pychemqt", "Accumulated fraction"), horizontalalignment='right', size='12')
...
if __name__ == "__main__":
    ...
    configuracion.read(config.conf_dir+"CheProcessrc")
