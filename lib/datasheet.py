...
class pdf():
    ...
    def bomba(self, bomba):
        ...
        self.c.drawString(
            3.6*cm, 600, "Bombas necesarias:  " + str(necesarios))
        ...
        self.c.drawString(11.6*cm, 540, "Caudal normal:  " +
                          str(caudal_normal) + "gdm")
        self.c.drawString(11.6*cm, 525, "Caudal de diseño:  " +
                          str(caudal_diseno) + "gdm")
        self.c.drawString(
            11.6*cm, 510, "Temperatura de bombeo:  " + str(temperatura) + "ºF")
        self.c.drawString(11.6*cm, 495, "Presión de vapor @ T.B.:  " +
                          representacion(presion) + config.Configuracion("Pressure").text())
        self.c.drawString(11.6*cm, 480, "Viscosidad @ T.B.:  " +
                          representacion(viscosidad) + config.Configuracion("Viscosity").text())
        self.c.drawString(
            11.6*cm, 465, "Gravedad específica @ T.B.:  " + str(gravedad))
    ...
    def ciclon(self, nombre, ciclon):
        ...
        self.c.drawCentredString(11.5*cm, 629, "General")
        self.c.setFont("Helvetica", 8)
        self.c.drawString(3.6*cm, 615, "Temperatura:  " + representacion(
            ciclon.entrada.T.config())+config.Configuracion("Temperature").text())
        self.c.drawString(3.6*cm, 600, "Fluido:  " + fluido)
        self.c.drawString(3.6*cm, 585, "Sólidos:  " + solidos)
        self.c.drawString(9.2*cm, 600, "Densidad @ T.P.:  " + representacion(
            ciclon.entrada.Gas.rho.config("DenGas"))+config.Configuracion("Density", "DenGas").text())
        self.c.drawString(14.2*cm, 600, "Viscosidad @ T.P.:  " + representacion(
            ciclon.entrada.Gas.mu.config())+config.Configuracion("Viscosity").text())
        self.c.drawString(9.2*cm, 585, "Densidad @ T.P.:  " + representacion(
            ciclon.entrada.solido.rho.config("DenLiq"))+config.Configuracion("Density", "DenLiq").text())
        self.c.drawString(3.6*cm, 570, "Caudal:  " + representacion(
            ciclon.entrada.Q.config("QGas"))+config.Configuracion("VolFlow", "QGas").text())
        self.c.drawString(11.6*cm, 570, "Eficiencia:  " +
                          representacion(ciclon.rendimiento))
        self.c.drawString(3.6*cm, 555, "Concentración sólidos entrada:  " + representacion(
            ciclon.entrada.solido.caudal.config())+config.Configuracion("MassFlow").text())
        self.c.drawString(11.6*cm, 555, "Concentración sólidos salida:  " + representacion(
            ciclon.SalidaAire.solido.caudal.config())+config.Configuracion("MassFlow").text())
        self.c.drawString(3.6*cm, 540, "Perdida de presión:  " + representacion(
            ciclon.DeltaP.config())+config.Configuracion("Pressure").text())
        ...
        self.c.setFont("Helvetica", 8)
        self.c.drawString(
            3.7*cm, 510, "Ciclones necesarios:  " + str(ciclon.num_ciclones))
        self.c.line(3.5*cm, 505, 10*cm, 505)
        self.c.line(3.5*cm, 490, 10*cm, 490)
        self.c.line(10*cm, 520, 10*cm, 490)
        self.c.drawString(3.7*cm, 495, "Caudal Unitario:  " + representacion(
            ciclon.entrada.Q.config("QGas"))+config.Configuracion("VolFlow", "QGas").text())
        self.c.drawString(6.8*cm, 465, "Dc:  " + representacion(
            ciclon.Dc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 450, "Bc:  " + representacion(
            ciclon.Bc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 435, "Hc:  " + representacion(
            ciclon.Hc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 420, "Jc:  " + representacion(
            ciclon.Jc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 405, "Lc:  " + representacion(
            ciclon.Lc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 390, "Zc:  " + representacion(
            ciclon.Zc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 375, "De:  " + representacion(
            ciclon.De.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 360, "Sc:  " + representacion(
            ciclon.Sc.config())+config.Configuracion("Length").text())
        self.c.drawImage(QtGui.QPixmap(
            os.environ["CheProcess"]+"/images/equip/ciclon_datasheet.gif"), 11*cm, 260, width=180, preserveAspectRatio=True)
        ...
        self.c.drawString(
            3.7*cm, 260, "Factor Instalación: " + str(ciclon.f_install))
        self.c.drawString(11.6*cm, 260, "Indice de costes: " +
                          str(ciclon.Current_index))
        self.c.drawString(
            3.7*cm, 245, "Coste Adquisición: $" + str(ciclon.C_adqTotal))
        self.c.drawString(
            11.6*cm, 245, "Coste Instalación: $" + str(ciclon.C_instTotal))
...
if __name__ == "__main__":
    pdf = pdf("CICLÓN")
    pdf.ciclon("Ciclon limpieza polvo", 12, 1.2, 1200, 1.84e-5, 126000, 93.6, 3.5, 99.56, 6500,
               1, 0.2, 0.5, 0.375, 1.5, 2.5, 0.5, 0.5, 10.2, "Industrial", 1.4, 549.2, 157088, 219924)
    pdf.dibujar()
